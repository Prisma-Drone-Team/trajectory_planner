...
void SPLINE_PLANNER::compute_traj()
{
	...
	for (int i = 0; i <= int(_times.size()) - 2; i++)
	...
	*/
	...
	for (double t = _times.front(), k = 0; t <= _times.back(); t += (1.0 / _freq))
	{ // CHECK TEMPO INIZIALE t=0
		if (t > _times[k + 1])
			k++;
...
void CARTESIAN_PLANNER::compute()
{
	...
	{
		// cout<<"Iter: "<<i<<endl;
		tf2::Quaternion qi(_poses[i].pose.orientation.x, _poses[i].pose.orientation.y, _poses[i].pose.orientation.z, _poses[i].pose.orientation.w);
		tf2::Quaternion qf(_poses[i + 1].pose.orientation.x, _poses[i + 1].pose.orientation.y, _poses[i + 1].pose.orientation.z, _poses[i + 1].pose.orientation.w);
		tf2::Matrix3x3 Ri_tf, Rf_tf;
		Matrix3d Ri, Rf;
		...
		if (abs(0.5 * (Rif(0, 0) + Rif(1, 1) + Rif(2, 2) - 1)) >= 1.0)
			xf = 0;
		Vector3d ri;
		if (xf != 0 && xf != M_PI)
		{
		...
		for (int j = 0; j < int(aplanner._x.size()); j++)
		{
			...
			tf2::Matrix3x3 Rb_des_tf;
			// tf2::matrixEigenToTF(Rb_des, Rb_des_tf);
			for (int i = 0; i < 3; i++)
			{
				for (int j = 0; j < 3; j++)
				{
					Rb_des_tf[i][j] = Rb_des(i, j);
				}
			}
	...
	for (int i = 0; i < int(xplanner._x.size()); i++)
	{
	...
	for (int i = int(xplanner._x.size()); i < int(_x.size()); i++)
	{ // Padding
		_x[i].header.frame_id = "iiwa_link_0";
		_xd[i].header.frame_id = "iiwa_link_0";
		_xdd[i].header.frame_id = "iiwa_link_0";
