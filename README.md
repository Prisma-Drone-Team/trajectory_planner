...
# Verify parameters
ros2 param list /offboard_control | grep joy
```

#### 2. Navigation Discontinuities
- Ensure SLAM is running: `ros2 topic echo /rtabmap/odom`
- Check transforms: `ros2 run tf2_tools view_frames`
- Verify waypoint tolerance in parameters

#### 3. PX4 Connection Issues
```bash
# Check MicroXRCE connection
ros2 topic list | grep fmu
ros2 service list | grep px4
...
### Adding New Commands
1. Add command parsing in `MoveManagerNode::pdt_callback()`
2. Implement command logic in `OffboardControl::move_cmd()`
3. Update keyboard interface in `OffboardControl::key_input()`

### Custom Trajectory Types
1. Extend `Trajectory` class with new trajectory type
2. Add trajectory generation in `planner.cpp`
3. Update trajectory selection logic

### Integration Examples
```cpp
// Service call example
auto client = node->create_client<std_srvs::srv::SetBool>("trajectory_service");
auto request = std::make_shared<std_srvs::srv::SetBool::Request>();
request->data = true;
client->async_send_request(request);

// Topic publishing example
auto pub = node->create_publisher<geometry_msgs::msg::PoseStamped>("waypoint", 10);
